...
var robot_alive_num int32
var robot_enter_scene_num int32
var all_robot_num int32
var begin_robot_time int64
...
func StartRobot(ips []IPInfo, acc string, password string, cnt uint32, bVip bool) {
	var i uint32
	wg.Add(int(cnt))
	all_robot_num = int32(cnt)
	DLog("all robot enter test. congratulations begin")
	begin_robot_time = time.Now().UnixMilli()
	...
	curr_alive_num := atomic.LoadInt32(&robot_alive_num)
	DLog("123456789 all robot begin exit %d", curr_alive_num)
...
func (robot *AIRobot) Tick() {
	...
	atomic.AddInt32(&robot_alive_num, 1)

	defer func() {
		robot.CloseNet()
		wg.Done()
		DLog("robot exit %d", robot.id)
		atomic.AddInt32(&robot_alive_num, -1)
	}()
TRY_AGAIN:
	robot.Start()
	if !robot.Connect() {
		DLog("robot connect fail and exit %d", robot.id)
		return
	}
	robot.C2S_Login()
	robot.bConnect = true
	robot.SetEnterSceneTimeout()
	for robot.IsRun() {
		...
		if !robot.enterScene {
			if robot.CheckEnterSceneTimeout() {
				DLog("robot enter scene timeout %d", robot.id)
				robot.CloseNet()
				robot.Stop()
			}
			continue
		...
		if robot.isLogin && robot.CheckHeartBeat() {
			robot.C2S_HeartBeat()
			robot.SetHeartBeat()
		}
	...
	time.Sleep(time.Second * time.Duration(rand.Int()%7))
	DLog("robot reconnect again %d", robot.id)
	if !robot.bStop {
		goto TRY_AGAIN
	}
}

func (robot *AIRobot) CheckEnterSceneTimeout() bool {
	tt := GetUnixTime()
	return tt > robot.enterSceneTime
}

func (robot *AIRobot) SetEnterSceneTimeout() {
	tt := GetUnixTime()
	robot.enterSceneTime = tt + 120
}

func (robot *AIRobot) CheckHeartBeat() bool {
	tt := GetUnixTime()
	return tt > robot.heartCnt
}

func (robot *AIRobot) SetHeartBeat() {
	tt := GetUnixTime()
	robot.heartCnt = tt + int64(rand.Int()%40) + 10
}
...
func (robot *AIRobot) Connect() bool {
	...
	for cnt > 0 {
		...
		if err != nil {
			DLog("Client connect error ! "+err.Error(), cnt)
...
func (robot *AIRobot) SendMsg() {
	for msg := range robot.sendChan {

		if msg == nil {
			return
		}

		robot.WriteN(int32(msg.index), msg.msg)
	}
}
...
func (robot *AIRobot) RecvData() *MsgInfo {
	...
	n, err := io.ReadFull(robot.Conn, head)
	if err != nil {
	...
	_index = binary.LittleEndian.Uint32(bindex)
	_len = binary.LittleEndian.Uint32(blen)

	if g_Handle.GetAction(_index) == nil {
		robot.Stop()
		DLog("%d invalid action %d and len %d", robot.id, _index, _len)
		return nil
	}

	if _len > 1024*1024 {
		robot.Stop()
		DLog("%d invalid index %d and len %d", robot.id, _index, _len)
		return nil
	}

	if !g_Handle.IsFather(_index) {
		buff := make([]byte, _len)
		n, err := io.ReadFull(robot.Conn, buff)
		if err != nil {
			robot.StopMsg("RecvData", err)
			return nil
		}
		...
		if n != int(_len) {
			DLog("%d invalid recv2  len %d:%d", robot.id, n, _len)
			if err != nil {
	...
	pkMsg := new(MsgInfo)
	pkMsg.index = _index
	pkMsg.msg = make([]byte, _len)

	n, err = io.ReadFull(robot.Conn, pkMsg.msg)
	if err != nil {
	...
	if n != int(_len) {
		DLog("%d invalid recv3  len %d %d", robot.id, n, _len)
		if err != nil {
...
func (robot *AIRobot) SendC2SData(id uint32, data []byte) {
	...
	msgdata := make([]byte, 20)
	var _flag uint32 = 0x11111111
	var _index uint32 = id
	var _len uint32 = (uint32)(len(data))
	var _para uint64 = 0
	binary.LittleEndian.PutUint32(msgdata, _flag)
	binary.LittleEndian.PutUint32(msgdata[4:], _index)
	binary.LittleEndian.PutUint32(msgdata[8:], _len)
	binary.LittleEndian.PutUint64(msgdata[12:], _para)
	msgdata = append(msgdata, data...)
	// DLog("index %d len %d %d", _index, _len, len(msgdata))

	robot.sendChan <- &MsgInfo{
		index: uint32(msgLength),
		msg:   msgdata[:],
	}
...
func (robot *AIRobot) Start() {
	robot.bRun = true
	robot.bConnect = false
	robot.exitChan = make(chan bool, 10)
	robot.MsgChan = make(chan *MsgInfo, 100)
	robot.sendChan = make(chan *MsgInfo, 100)
	robot.enterScene = false
	robot.isLogin = false
...
func S2C_Login(ai *AIRobot, msg []byte) {
	...
	DLog("robot %d login ack", ai.id)
	err := proto.Unmarshal(msg, &skData)
	if err != nil {
		ai.StopMsg("S2C_Login", err)
	} else {
		ai.isLogin = true
		DLog("robot %d %d login ack", ai.id, skData.GetRetCode())
...
func S2C_RoleInfo(ai *AIRobot, msg []byte) {
	...
	if err != nil {
		...
		} else if skData.GetRetCode() == pbd.E_MSG_RET_CODE_Msg_Ret_Code_Player_Was_OnLine {
...
func S2C_RoleInfo_EnterScene_Action(ai *AIRobot, pkData *pbd.S2C_RoleInfo_EnterScene) {
	...
	if ai.enterScene == false {
		ai.enterScene = true
		atomic.AddInt32(&robot_enter_scene_num, 1)
		num := atomic.LoadInt32(&robot_enter_scene_num)
		DLog("curr enter scene num %d", num)
		if num == all_robot_num {
			DLog("all robot enter scene. congratulations %d %d", all_robot_num, time.Now().UnixMilli()-begin_robot_time)
		}
	}
...
func S2C_Chat(ai *AIRobot, msg []byte) {
	var skData pbd.S2C_Chat
	err := proto.Unmarshal(msg, &skData)
	if err != nil {
		ai.StopMsg("S2C_Chat", err)
	} else {
		DLog("world has %d people", skData.GetSceneInstanceId())
	}
}
